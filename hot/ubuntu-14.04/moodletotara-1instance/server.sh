...
echo "host    $SITEENVIRONMENT $SITEENVIRONMENT             127.0.0.1/32        md5" >> /etc/postgresql/9.3/main/pg_hba.conf
...
if [[ $APPVERSION < 29 ]]
then
 sudo -u postgres psql  -d $SITEENVIRONMENT -c "update mdl_user set email='null@catalyst.net.nz' where username LIKE 'admin'"
fi
# Install nginx
